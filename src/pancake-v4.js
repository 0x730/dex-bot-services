...
function getPrivateKey(label = 'MAIN', chain) {
  return getWalletPrivateKey('PANCAKE_V4', label, chain);
}
...
function initializePancakeV4TxContext(chain, walletLabel = 'MAIN') {
  ...
  const maxGas = readFirstIntEnv(
    [`PANCAKE_V4_MAX_GAS_${upper}`, 'MAX_GAS_ETHEREUM'],
    1000000
  );
...
registerServiceHealth(app, 'pancake-v4');
...
app.post('/pancake-v4-approve', async (req, res) => {
  ...
  try {
    ...
    logger.error('pancake-v4-approve failed:', error);
...
function startServer() {
  ...
  reportServiceBind('pancake-v4', HOST, PORT);
