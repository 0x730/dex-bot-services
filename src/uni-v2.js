...
function getPrivateKey(label = 'MAIN', chain) {
  return getWalletPrivateKey('UNI_V2', label, chain);
}
...
function getChainObjects(chain, walletLabel = 'MAIN') {
  ...
  logger.info(`Initialized Router Address: ${v2Router.target}`);
...
async function checkAllowance(
  ...
  try {
    ...
    if (currentAllowance < amountInWei) {
      logger.info(`Insufficient allowance. Approving router to spend tokens.`);
      return false;
    }
    return true;
  } catch (error) {
...
async function buyTokenV2(
  ...
  logger.info(`Amount Out Estimate: ${amountOutEstimate.toString()}`);
...
async function sellTokenV2(
  ...
  logger.info(`Attempting to sell ${amountInTokens} tokens on ${chain} chain.`);
  ...
  try {
    ...
    logger.info(`Amount to Sell (wei): ${amountInWei.toString()}`);
    ...
    if (!hasSufficientAllowance) {
      ...
      logger.info(`Approved router ${v2Router.target} to spend tokens.`);
    } else {
...
async function estimateBuyV2Cost(
  ...
  try {
    ...
    logger.info(`Total Cost (ETH): ${totalCostEth}`);
...
async function estimateSellV2Cost(
  ...
  const amountOutEstimate = await getV2Quote(v2Router, path, amountInWei);
  logger.info('amountOutEstimate: ', amountOutEstimate);
  const sp = BigInt(100 - slippagePercent);
  const hundred = BigInt(100);
  const amountOutMin = (BigInt(amountOutEstimate.toString()) * sp) / hundred;
  const deadline = Math.floor(Date.now() / 1000) + 60 * 20;
  ...
  const outputIsETH =
    path[path.length - 1].toLowerCase() === wethAddress.toLowerCase();
  logger.info('outputIsETH: ', outputIsETH);
...
registerServiceHealth(app, 'uni-v2');
...
app.post('/uni-v2-buy', async (req, res) => {
  ...
  try {
    ...
    logger.error('V2 Buy failed:', error);
    if (
...
app.post('/uni-v2-sell', async (req, res) => {
  ...
  try {
    ...
    logger.info(`Received Amount: ${receivedAmountRaw}`);
    return res.json({
    ...
    logger.error('V2 Sell failed:', error);
    if (
      error.message.includes('exceeds the maximum allowed gas') ||
      error.code === 'INSUFFICIENT_FUNDS'
    ) {
...
app.post('/uni-v2-swap', async (req, res) => {
  ...
  try {
    ...
    logger.error('V2 Token-to-Token Swap failed:', error);
    if (
      error.message.includes('exceeds the maximum allowed gas') ||
      error.code === 'INSUFFICIENT_FUNDS'
    ) {
...
app.post('/uni-v2-estimate-buy-cost', async (req, res) => {
  ...
  try {
    ...
    logger.error('Estimate buy cost failed:', error);
    return res
      .status(500)
...
app.post('/uni-v2-estimate-sell-cost', async (req, res) => {
  ...
  try {
    ...
    logger.error('Estimate sell cost failed:', error);
    return res
      .status(500)
...
app.post('/uni-v2-estimate-swap-cost', async (req, res) => {
  ...
  try {
    ...
    logger.error('Estimate swap cost failed:', error);
    return res
      .status(500)
...
function startServer() {
  ...
  reportServiceBind('uni-v2', HOST, PORT);
