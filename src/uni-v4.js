...
function getPrivateKey(label = 'MAIN', chain) {
  const envVar = `UNI_V4_PRIVATE_KEY_${label.toUpperCase()}_${chain.toUpperCase()}`;
...
function initializeUniV4TxContext(chain, walletLabel = 'MAIN') {
  ...
  const maxGas = parseInt(process.env.MAX_GAS_ETHEREUM) || 1000000;
...
async function approvePermit2SpenderIfNeeded(
  ...
  if (!needsAmount && !needsExpiration) {
    console.log(`Permit2 allowance already set for spender ${spenderAddress}`);
    return;
  }
...
async function quoteExactInputSingle({
  ...
  try {
    ...
    console.error(`[DEBUG] Quote failed: ${err.message}`);
    throw err;
  }
}
...
app.post('/uni-v4-quote-exact-in-single', async (req, res) => {
  ...
  try {
    ...
    console.error('Uni v4 quote failed:', error);
    return res.status(500).json({
...
app.post('/uni-v4-build-calldata', async (req, res) => {
  ...
  try {
    ...
    console.error('Uni v4 build calldata failed:', error);
    return res.status(500).json({
