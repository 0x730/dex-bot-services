...
function getPrivateKey(label = 'MAIN', chain) {
  return getWalletPrivateKey('PANCAKE_V3', label, chain);
}
...
function initializePancakeV3(chain, walletLabel = 'MAIN') {
  const config = {
    ...
    maxGas: readFirstIntEnv(
      [`PANCAKE_V3_MAX_GAS_${chain.toUpperCase()}`],
      200000
    ),
  };
...
async function getBnbBalance(provider, address) {
  try {
    ...
    logger.error(`Error getting BNB balance for ${address}:`, error);
    throw error;
  }
}
...
async function unwrapWBNB(provider, signer, wbnbAddress) {
  try {
    ...
    if (wbnbBalance === 0n) {
      logger.info('No WBNB balance to unwrap.');
      return;
    }
    ...
    logger.error(`Error unwrapping WBNB to BNB:`, error);
    throw error;
  }
}
...
registerServiceHealth(app, 'pancake-v3');
...
app.post('/pancake-v3-approve', async (req, res) => {
  ...
  try {
    ...
    logger.error('pancake-v3-approve failed:', error);
...
app.post('/pancake-v3-buy', async (req, res) => {
  ...
  try {
    ...
    logger.info('Buy TX confirmed:', receipt.hash);
    ...
    logger.error('pancake-v3-buy failed:', error);
    res
      .status(500)
...
app.post('/pancake-v3-sell', async (req, res) => {
  ...
  try {
    ...
    const receipt = await tx.wait();
    logger.info('Sell TX confirmed:', receipt.hash);
    ...
    logger.error('pancake-v3-sell failed:', error);
    res
      .status(500)
...
app.post('/pancake-v3-swap', async (req, res) => {
  ...
  try {
    ...
    logger.error('Pancake V3 Swap failed:', error);
...
app.post('/pancake-v3-estimate-buy-cost', async (req, res) => {
  ...
  try {
    ...
    logger.error('Estimate Pancake V3 Buy cost failed:', error);
    return res.status(500).json({
...
app.post('/pancake-v3-estimate-sell-cost', async (req, res) => {
  ...
  try {
    ...
    logger.error('Estimate Pancake V3 Sell cost failed:', error);
    return res.status(500).json({
...
app.post('/pancake-v3-estimate-swap-cost', async (req, res) => {
  ...
  try {
    ...
    logger.error('Estimate Pancake V3 Swap failed:', error);
    return res.status(500).json({
...
function startServer() {
  ...
  reportServiceBind('pancake-v3', HOST, PORT);
