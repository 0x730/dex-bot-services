...
function getPrivateKey(label = 'MAIN', chain) {
  const envVar = `UNI_V3_PRIVATE_KEY_${label.toUpperCase()}_${chain.toUpperCase()}`;
...
async function unwrapWMATIC(provider, signer, wmAddress) {
  try {
    ...
    if (wmBalance === 0n) {
      console.log('No WMATIC to unwrap.');
      return;
    }
    ...
    console.error(`Error unwrapping WMATIC to MATIC:`, error);
    throw error;
  }
}
...
async function unwrapBase(provider, signer, baseWethAddress) {
  try {
    ...
    console.error(`Error unwrapping WETH to BASE:`, error);
    throw error;
  }
}
...
supportedChains.forEach((chain) => {
  try {
    ...
    console.error(
      `Failed to initialize contracts for ${chain}:`,
      error.message
    );
  }
});
...
app.post('/uni-v3-buy', async (req, res) => {
  ...
  try {
    ...
    const feeTier =
      parseInt(fee) || parseInt(process.env.UNI_V3_FEE_TIER_DEFAULT) || 3000;
    ...
    console.error('Uniswap V3 Buy failed:', error);
    return res
      .status(500)
...
app.post('/uni-v3-sell', async (req, res) => {
  ...
  try {
    ...
    if (wrappedAddrToCheck) {
      ...
      if (wrappedBal > 0n) {
        try {
          ...
          console.error('[UniV3] Unwrap failed:', unwrapErr);
        }
      } else {
        console.log('[UniV3] No wrapped native balance to unwrap on chain', lc);
      }
    } else {
      console.warn('[UniV3] Unknown chain for unwrap:', lc);
    }
    ...
    console.error('Uniswap V3 Sell failed:', error);
    return res
      .status(500)
...
app.post('/uni-v3-estimate-buy-cost', async (req, res) => {
  ...
  try {
    ...
    console.error('Estimate Uniswap V3 Buy cost failed:', error);
...
app.post('/uni-v3-estimate-sell-cost', async (req, res) => {
  ...
  try {
    ...
    console.error('Estimate Uniswap V3 Sell cost failed:', error);
    return res.status(500).json({
...
app.post('/uni-v3-estimate-swap-cost', async (req, res) => {
  ...
  try {
    ...
    console.error('Estimate Uniswap V3 Swap cost failed:', error);
    return res
      .status(500)
